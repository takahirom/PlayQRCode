...
import com.google.zxing.client.result.CalendarBarcode;
...
public final class CalendarResultHandler extends ResultHandler {
  ...
  public CalendarResultHandler(Activity activity, Barcode result) {
  ...
  public void handleButtonPress(int index) {
    if (index == 0) {
      CalendarBarcode calendarResult = (CalendarBarcode) getResult();
  ...
  public CharSequence getDisplayContents() {
    ...
    CalendarBarcode calResult = (CalendarBarcode) getResult();
    StringBuilder result = new StringBuilder(100);

    Barcode.maybeAppend(calResult.getSummary(), result);

    Date start = calResult.getStart();
    Barcode.maybeAppend(format(calResult.isStartAllDay(), start), result);
    ...
    if (end != null) {
      ...
      Barcode.maybeAppend(format(calResult.isEndAllDay(), end), result);
    }

    Barcode.maybeAppend(calResult.getLocation(), result);
    Barcode.maybeAppend(calResult.getOrganizer(), result);
    Barcode.maybeAppend(calResult.getAttendees(), result);
    Barcode.maybeAppend(calResult.getDescription(), result);
