...
import com.google.android.gms.vision.barcode.Barcode;
import com.google.android.gms.vision.barcode.BarcodeType;
...
public abstract class ResultHandler {
  ...
  ResultHandler(Activity activity, Barcode barcode) {
    this.activity = activity;
    this.barcode = barcode;
  }

  public final Barcode getResult() {
    return barcode;
  }
  ...
  final void addPhoneOnlyContact(Barcode.Phone[] phoneNumbers) {
    addContact(null,  null, phoneNumbers, null, null, null,  null, null, null, null, null, null);
  }

  final void addEmailOnlyContact(Barcode.Email[] emails) {
    addContact(null,  null, null, emails, null, null, null,  null, null, null, null, null);
  }

  final void addContact(Barcode.PersonName names,
//                        String nicknames,
                        String pronunciation,
                        Barcode.Phone[] phoneNumbers,
                        Barcode.Email[] emails,
                        String note,
    ...
    putExtra(intent, ContactsContract.Intents.Insert.NAME, names != null ? names : null);
    ...
    for (int x = 0; x < phoneCount; x++) {
      putExtra(intent, Contents.PHONE_KEYS[x], phoneNumbers[x].number);
      int type = phoneNumbers[x].type;
      if (type >= 0) {
    ...
    for (int x = 0; x < emailCount; x++) {
      putExtra(intent, Contents.EMAIL_KEYS[x], emails[x].address);
      int type = emails[x].type;
      if (type >= 0) {
...
//    }
    ...
    if (address != null) {
      putExtra(intent, ContactsContract.Intents.Insert.POSTAL, address.addressLines[0]);
  ...
  final void searchBookContents(String isbnOrUrl) {
    // TODO: BOOK Search
    Toast.makeText(activity, "book search", Toast.LENGTH_SHORT).show();
